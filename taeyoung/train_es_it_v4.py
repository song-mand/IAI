...
- Korean is not trained as a generation model in v4.
- Korean is handled in sub_v4_es_it_ko.py by:
  1) copy/change classifier,
  2) train-derived candidate dictionary,
  3) jamo-distance candidate retrieval,
  4) conservative candidate ranking.
"""

TARGET_LANGUAGES = ["es", "it"]
PROMPT_TYPE = {"es": "marked_natural", "it": "marked_natural"}


class LexNormDataset(Dataset):
    def __init__(self, split="train", target_lang="es", tokenizer_name="google/byt5-small", max_length=128, prompt_type="marked_natural"):
        ...
        for row in raw_data:
            ...
            for i, raw_word in enumerate(raw_words):
                ...
                if prompt_type == "marked_natural":
                    input_text = f"normalize lang: {lang} target: {raw_word} context: {marked_context}"
                elif prompt_type == "natural":
                    input_text = f"lang: {lang} word: {raw_word} context: {plain_context}"
...
def load_base_model(lang):
    candidates = [f"ufal/byt5-small-multilexnorm2021-{lang}", "google/byt5-small"]
    last_error = None
...
def train_one_language(lang):
    ...
    training_args = Seq2SeqTrainingArguments(
        output_dir=f"./models/byt5-{lang}-v4",
        save_strategy="no",
        learning_rate=3e-5,
