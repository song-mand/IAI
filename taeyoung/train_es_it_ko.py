...
def load_base_model(lang):
    """
    es/it: use language-specific ufal checkpoint first if available.
    ko: use google/byt5-small because a ko-specific ufal checkpoint may not exist.
    """
    if lang in ["es", "it"]:
        candidates = [
...
def build_lora_model(model):
    """
    PEFT versions differ. Try all-linear first, then fall back to q/v modules.
    """
    try:
        ...
        print("[LoRA] all-linear failed. Fallback to ['q', 'v']")
        print(e)
...
def get_train_setting(lang):
    """
    ko starts from google/byt5-small, so train a bit longer.
    es/it may use existing lexical normalization checkpoints, so 2 epochs is a safer start.
    """
    if lang == "ko":
        return {
            "epochs": 3,
...
def train_one_language(lang):
    ...
    training_args = Seq2SeqTrainingArguments(
        output_dir=f"./models/byt5-{lang}-corrected",
        save_strategy="no",
