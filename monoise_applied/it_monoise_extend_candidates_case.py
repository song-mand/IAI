...
def case_variants(raw: str):
    """
    Explicit case-only variants.
    ...
    These are candidates only. The accept-override model decides
    whether a case-only candidate is safe to use.
    """
    variants = set()
    ...
    # Avoid protected-looking tokens.
    ...
    variants.add(raw.capitalize())

    variants.discard(raw)
    return {v for v in variants if v and v != raw}
...
class ITCandidateGenerator:
    ...
    def generate(self, raw: str):
        ...
        # Explicit case-variant candidates.
        # They improve recall for case-only gold targets such as tokio -> Tokio,
        # SONY -> Sony, ITALIA -> Italia.
        for cand in case_variants(raw):
            self._add(candidates, cand, "case_variant")
...
def feature_dict(raw: str, cand: str, left: str, right: str, sources, gen: ITCandidateGenerator):
    ...
    feats = {
        ...
        "from_case": int("case" in sources or "case_variant" in sources),
        "from_case_variant": int("case_variant" in sources),
