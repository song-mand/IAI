...
def extract_features(raw: str, left: str, right: str, raw_stats, key_stats):
    t = raw
    low = t.lower()
    key = make_key(t)
...
def build_examples(rows, raw_stats, key_stats):
    X = []
    y = []
    ...
    for row in rows:
        ...
        for i, raw in enumerate(raw_words):
            ...
            X.append(extract_features(raw, left, right, raw_stats, key_stats))
            y.append(label)
...
def main():
    ...
    parser.add_argument("--output", type=str, default="./detectors/es_change_detector_rf.joblib")
    ...
    parser.add_argument("--change_weight", type=float, default=3.0)
    ...
    raw_stats, key_stats = build_stats(train_data)
    X, y = build_examples(train_data, raw_stats, key_stats)
