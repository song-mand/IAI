...
# Allow importing train_es_change_detector.py when this script is placed in scripts/.
SCRIPT_DIR = Path(__file__).resolve().parent
if str(SCRIPT_DIR) not in sys.path:
    sys.path.insert(0, str(SCRIPT_DIR))

from train_es_change_detector import extract_features, LANG


def build_valid_examples(rows, raw_stats, key_stats):
    X = []
    y = []
    tokens = []
...
def main():
    ...
    parser.add_argument("--detector", type=str, default="./detectors/es_change_detector.joblib")
    ...
    X, y, tokens = build_valid_examples(valid_data, raw_stats, key_stats)
