...
# Case-aware modules.
BASE_MODULE="monoise_applied.it_monoise_extend_candidates_case"
ACCEPT_MODULE="monoise_applied.it_monoise_accept_override_case"
...
SPLIT_DIR="./eval_splits_it_case_accept"
SPLIT_PREFIX="it"
...
BASE_MODEL_DIR="./models_it_monoise_case_candidates"
BASE_MODEL="${BASE_MODEL_DIR}/it_monoise_extend_candidates_case.joblib"

ACCEPT_MODEL_DIR="./models_it_monoise_case_accept_override"
ACCEPT_MODEL="${ACCEPT_MODEL_DIR}/it_case_accept_override.joblib"

REPORT_DIR="./reports"
REPORT_PATH="${REPORT_DIR}/it_case_accept_override_eval.txt"
...
# CASE_OVERCHANGE_NEGATIVE_WEIGHT:
# Weight for wrong case-only changes.
# Higher value = prevents errors like Tempo -> tempo, Lei -> lei.
CASE_OVERCHANGE_NEGATIVE_WEIGHT=40.0

# CASE_POSITIVE_WEIGHT:
# Weight for correct case-only changes.
# Higher value = encourages corrections like tokio -> Tokio, SONY -> Sony.
CASE_POSITIVE_WEIGHT=3.5
...
if [ ! -f "./monoise_applied/it_monoise_extend_candidates_case.py" ]; then
  echo "ERROR: base module file not found: ./monoise_applied/it_monoise_extend_candidates_case.py"
  exit 1
fi

if [ ! -f "./monoise_applied/it_monoise_accept_override_case.py" ]; then
  echo "ERROR: accept module file not found: ./monoise_applied/it_monoise_accept_override_case.py"
  exit 1
fi

echo "=============================="
echo "1. Remove previous case-aware accept override experiment files"
echo "=============================="
...
echo "=============================="
echo "3. Train case-aware MoNoise extend-candidates ranker"
echo "=============================="
...
echo "=============================="
echo "4. Train case-aware accept override model"
echo "=============================="
...
run_module "$ACCEPT_MODULE" train \
  ...
  --case_overchange_negative_weight "$CASE_OVERCHANGE_NEGATIVE_WEIGHT" \
  --case_positive_weight "$CASE_POSITIVE_WEIGHT" \
  --seed "$SEED"

echo "=============================="
echo "5. Evaluate case-aware accept override model"
echo "=============================="
...
# Optional threshold-only ablation after the model is trained:
:<<'END'
python -m monoise_applied.it_monoise_accept_override_case eval \
  --train_file ./eval_splits_it_case_accept/it_train.parquet \
  --valid_file ./eval_splits_it_case_accept/it_valid.parquet \
  --model ./models_it_monoise_case_accept_override/it_case_accept_override.joblib \
  --accept_threshold 0.80 \
  --verbose | tee ./reports/it_case_accept_override_eval_t080.txt
...
python -m monoise_applied.it_monoise_accept_override_case eval \
  ...
  --verbose | tee ./reports/it_case_accept_override_eval_t085.txt
END
